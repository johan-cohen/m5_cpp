...
PktSubAck::PktSubAck() : properties(PktType::SUBACK)
{
}

PktSubAck::PktSubAck(AppBuf &buf) : properties(PktType::SUBACK)
{
	this->readFrom(buf);
}


uint32_t PktSubAck::writeTo(AppBuf &buf)
{
	...
	buf.writeNum8((uint8_t)PktType::SUBACK << 4);
	buf.writeVBI(remLen);
...
uint32_t PktSubAck::readFrom(AppBuf &buf)
{
	...
	first = buf.readNum8();
	if (first != ((uint8_t)PktType::SUBACK << 4)) {
