...
#ifndef __PROPERTY_NODE_HPP__
#define __PROPERTY_NODE_HPP__
...
public:
	...
	PropertyNode(uint8_t id = 0, const uint8_t *data = nullptr, uint16_t size = 0):
		value(data, size)
	{
		_id = id;
	}

	PropertyNode(uint8_t id, uint64_t v) : value(v)
	{
		_id = id;
	}
